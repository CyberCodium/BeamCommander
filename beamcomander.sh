...
configure_wireless() {
    while true; do
        header "Wireless Configuration (5 GHz)"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 19. GPIO / HARDWARE CONTROL
# ═══════════════════════════════════════════════════════════════════════════════
hardware_control() {
    while true; do
        header "Hardware Control"

        echo "1. Show GPIO status"
        echo "2. Toggle GPIO pin"
        echo "3. Show hardware info"
        echo "4. Show thermal sensors"
        echo "5. Show voltage/power info"
        echo "6. Ethernet speed/duplex"
        ...
        case $choice in
            1) ssh_cmd "cat /sys/class/gpio/*/value 2>/dev/null; ls /sys/class/gpio/ 2>/dev/null" ;;
            2)
                read -p "GPIO pin number: " pin
                if validate_int "$pin" 0 100; then
                    ssh_cmd "echo $pin > /sys/class/gpio/export 2>/dev/null; echo out > /sys/class/gpio/gpio${pin}/direction 2>/dev/null; cat /sys/class/gpio/gpio${pin}/value" 
                    read -p "Set value (0/1): " val
                    ssh_cmd "echo $val > /sys/class/gpio/gpio${pin}/value"
                    success "GPIO $pin set to $val"
                fi
                ;;
            3)
                ssh_cmd "cat /tmp/sysinfo/board_name 2>/dev/null; cat /tmp/sysinfo/model 2>/dev/null; cat /proc/cpuinfo 2>/dev/null"
                ;;
            4)
                ssh_cmd "for tz in /sys/class/thermal/thermal_zone*/temp; do echo \"\$tz: \$(cat \$tz 2>/dev/null)m°C\"; done 2>/dev/null" || echo "No thermal sensors"
                ;;
            5) ssh_cmd "cat /sys/class/power_supply/*/uevent 2>/dev/null" || echo "No power info available" ;;
            6)
                ssh_cmd "ethtool eth0 2>/dev/null || cat /sys/class/net/eth0/speed 2>/dev/null"
                echo ""
                echo "1. Force 100 Mbps full duplex"
                echo "2. Force 1000 Mbps (Gigabit)"
                echo "3. Auto-negotiate"
                read -p "Select (or Enter to skip): " ec
                case $ec in
                    1) ssh_cmd "ethtool -s eth0 speed 100 duplex full autoneg off 2>/dev/null"; success "Set to 100 Mbps FD" ;;
                    2) ssh_cmd "ethtool -s eth0 speed 1000 duplex full autoneg off 2>/dev/null"; success "Set to 1000 Mbps FD" ;;
                    3) ssh_cmd "ethtool -s eth0 autoneg on 2>/dev/null"; success "Auto-negotiate enabled" ;;
                esac
                ;;
            7)
                log "Press the reset button on the device..."
                ssh_cmd "cat /proc/interrupts | grep -i button; evtest /dev/input/event0 2>/dev/null" || echo "Button test not available"
                ;;
            0) return ;;
        esac
...
show_menu() {
    ...
    echo "  2. Logging & Diagnostics"
    echo ""
    echo -e " ${CYAN}── WIRELESS ──${NC}"
    echo "  3. Wireless Configuration (20 options)"
    echo "  4. Antenna Alignment"
    echo "  5. Monitor Mode & Packet Capture"
    echo ""
    echo -e " ${CYAN}── NETWORK ──${NC}"
    echo "  6. Network Configuration (15 options)"
    echo "  7. DHCP Configuration"
    echo "  8. Firewall Configuration (13 options)"
    echo "  9. QoS / Traffic Shaping"
    echo " 10. Network Testing (11 tests)"
    echo ""
    echo -e " ${CYAN}── MANAGEMENT ──${NC}"
    echo " 11. Firmware & Packages"
    echo " 12. Backup & Restore"
    echo " 13. Scheduled Tasks (Cron)"
    echo " 14. LED Management"
    echo " 15. MAC Address Management"
    echo " 16. Security Hardening"
    echo " 17. Services Management"
    echo " 18. Hardware / GPIO Control"
    echo ""
    echo -e " ${CYAN}── ACCESS ──${NC}"
    echo " 19. SSH Terminal"
    echo " 20. Reboot Device"
    echo " 21. Change Target IP"
    echo ""
    echo "  0. Exit"
    echo ""
}
...
main() {
    ...
    while true; do
        ...
        case $choice in
            1)  get_system_info ;;
            2)  logging_diagnostics ;;
            3)  configure_wireless ;;
            4)  antenna_alignment ;;
            5)  monitor_mode ;;
            6)  configure_network ;;
            7)  configure_dhcp ;;
